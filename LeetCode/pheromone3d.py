...
def generatePath(x_initial_pos,y_initial_pos,z_initial_pos):
	t , c_state = logistic(10**7,10**6)
	X = modulo(c_state)
	len = np.size(X)
	x = np.empty(len,dtype = object)
	y = np.empty(len,dtype = object)
	z = np.empty(len,dtype = object)
	x[0] = x_initial_pos
	y[0] = y_initial_pos
	z[0] = z_initial_pos
	for i in range(0,len-1):
		x[i+1], y[i+1], z[i+1] = motion_direction(x[i],y[i],z[i],X[i])
	return x, y
x, y, z = generatePath(25,25,25)
plt.plot3(x,y,z)
